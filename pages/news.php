...
$req = $cnx->prepare('SELECT * FROM lire_debut_articles()');
    ...
    foreach( $result as $row ){
        ...
        <li class="listeArticles">
            <a href="#" class="lienArticle" data-type="news" data-id="<?php echo $row['id'];?>"><h3 class="titreArticle"><?php echo $row['titre'];?></h3></a>
            <?php
            $pos1 = stripos($row['texte'], '<p>');
            $chaine=substr($row['texte'], $pos1);
            ?>
            <a href="#" class="lienArticle" data-type="news" data-id="<?php echo $row['id'];?>">
